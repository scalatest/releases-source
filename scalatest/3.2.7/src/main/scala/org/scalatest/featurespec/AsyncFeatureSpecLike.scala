...
trait AsyncFeatureSpecLike extends AsyncTestSuite with AsyncTestRegistration with Informing with Notifying with Alerting with Documenting { thisSuite =>
   ...
   * <br/><span class="stReserved">class</span> <span class="stType">StackSpec</span> <span class="stReserved">extends</span> <span class="stType">AsyncFeatureSpec</span> {
   *   <span class="stType">Feature</span>(<span class="stQuotedString">"A Stack"</span>) {
   *     <span class="stType">Scenario</span>(<span class="stQuotedString">"(when not empty) must allow me to pop"</span>) { succeed }
   *     <span class="stType">Scenario</span>(<span class="stQuotedString">"(when not full) must allow me to push"</span>) { succeed }
   *   }
   * }
   * </pre>
   *
